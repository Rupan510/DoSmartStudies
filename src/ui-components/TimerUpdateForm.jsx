...
export default function TimerUpdateForm(props) {
  const {
    id: idProp,
    timer,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = timerRecord
      ? { ...initialValues, ...timerRecord }
      : initialValues;
    setStart(cleanValues.start);
    setEnd(cleanValues.end);
    setErrors({});
  };
  const [timerRecord, setTimerRecord] = React.useState(timer);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Timer, idProp) : timer;
      setTimerRecord(record);
    };
    queryData();
  }, [idProp, timer]);
  React.useEffect(resetStateValues, [timerRecord]);
  const validations = {
    start: [],
    end: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Timer.copyOf(timerRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TimerUpdateForm")}
      {...rest}
    >
      <TextField
        label="Start"
