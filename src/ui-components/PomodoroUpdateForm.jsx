...
export default function PomodoroUpdateForm(props) {
  const {
    id: idProp,
    pomodoro,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = pomodoroRecord
      ? { ...initialValues, ...pomodoroRecord }
      : initialValues;
    setStart(cleanValues.start);
    setEnd(cleanValues.end);
    setErrors({});
  };
  const [pomodoroRecord, setPomodoroRecord] = React.useState(pomodoro);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Pomodoro, idProp)
        : pomodoro;
      setPomodoroRecord(record);
    };
    queryData();
  }, [idProp, pomodoro]);
  React.useEffect(resetStateValues, [pomodoroRecord]);
  const validations = {
    start: [],
    end: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Pomodoro.copyOf(pomodoroRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PomodoroUpdateForm")}
      {...rest}
    >
      <TextField
        label="Start"
